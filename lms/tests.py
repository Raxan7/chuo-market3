...
class LMSModuleGatingTests(TestCase):
    ...
    def test_default_order_modules_still_lock_later_modules(self):
        first_module = CourseModule.objects.create(
            course=self.course,
            title='Module 1',
            description='First module',
        ...
        self.assertFalse(response.context['module_states'][1]['unlocked'])
        self.assertEqual(response.context['module_states'][0]['module'].id, first_module.id)
        self.assertEqual(response.context['module_states'][1]['module'].id, second_module.id)
